...
def clear():
      if os.name=='nt':
          os.system('cls && title [NEW] NOVA - FULL KEY GENERAT0R BY NOVA ')
      else :
          os.system('clear')
#------------#
def strounga(x,y):
...
logo=choice(["""{w}
   ^     . .     ^  
   \ \    V    / /  
   \   \(+ +)/   / {r}({y}Instagram{r}){rr}~~~~~~~~~{r}({y}@mr_hack93{r}){w} 
   \   (  v  )   / {r}({y}Telegram{r}){rr}~~~~~~~~~~{r}({y}@hack151{r}){w}
    \____m_m____/   {r}({y}linktr.ee{r}/{y}NOVA{r}){rr}~~{r}({y}2021{r}){w}
   <[___________]>
...
d88888P'`?888P'd88'     `?8888P'`?888P'`?88P'?8b `?888P'
                                             ...
                                             ,d8P       
                                          `?888P'       
{rr}*{r} Instagam     {rr}:{y}@mr_hack93
{rr}*{r} Telegram     {rr}:{y}@hack151
{rr}*{r} Last Update  {rr}:{y}Nova{rr}/{y}2021
...
{rr}\\_{r}Last-Update{rr}:{y}Nova{rr}/{y}2021
...
class z4ck(object):
  ...
  def __init__(self):
      ...
      baka=input('\033[91m[\033[93NOVA>\033[91m]\033[00m ')
      while baka=='':
        baka=input('\033[91m[\033[93NOVA>\033[91m]\033[00m ')
      baka=int(baka)
      while baka <1 or baka >(xnumx+7):
         baka=int(input('\033[91m[\033[93NOVA>\033[91m]\033[00m '))
      if baka<=xnumx:
...
#Key creator 
  ...
  def delmyconf(self):
    ...
    open('config.ini','w').write('#Free Keys by @NOVA\n')
    for i in configs :
      open('config.ini','a').write('%s::%s\n'%(i['name'],i['format']))
  def seemydick(self):
   print("""{r}
     ({y}Name/Nickname{r}){rr}~~{r}({y}Wounded{rr}/{y}NOVA{r}) 
